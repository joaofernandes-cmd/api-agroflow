...
| **Como será atendido** | Armazenamento persistente via Supabase (PostgreSQL), fila de sincronização implementada no backend Node.js com retry automático, confirmação de envio com resposta do servidor, versionamento de registros com timestamp, logs de auditoria no banco e tela de status de sincronização visível ao usuário. |
...
| **Como será atendido** | Assets otimizados e lazy loading de componentes, dados armazenados localmente no navegador no modo offline, backend Node.js com TypeScript para processamento das requisições via Supabase (PostgreSQL). |
...
| **Como será atendido** | Arquitetura web centralizada (Node.js + Supabase), deploy via CI/CD automatizado (GitLab CI), logs acessíveis via painel do Supabase, monitoramento via ferramentas nativas da plataforma. |
...
| **Como será atendido** | RBAC implementado no backend Node.js com TypeScript (middleware de autorização por perfil e retiro), autenticação via Supabase Auth (JWT), isolamento de dados via Row Level Security (RLS) no PostgreSQL (queries com `WHERE retiro_id`), validação de permissões em cada endpoint, auditoria com trigger de banco registrando todas as operações, criptografia TLS gerenciada pelo Supabase. |
...
| **Como será atendido** | Infraestrutura gerenciada pelo Supabase (PostgreSQL hospedado), backend Node.js com TypeScript, índices nas colunas críticas (`retiro_id`, `criado_em`, `sincronizado`), particionamento lógico de dados por retiro, rate limiting no backend. |
...
| Campo | Conteúdo |
|:--|:--|
| **Requisito Não Funcional** | A identidade visual do sistema deve seguir a paleta de cores e tipografia definidas pela equipe de desenvolvimento em alinhamento com o parceiro BrPec Agropecuária, uma vez que a empresa não possui manual de marca formalizado. A aplicação deve ser exclusivamente web (não nativa), compatível com navegadores modernos (Chrome, Edge, Safari) e responsiva para tablets Android. |
| **Métrica / Critério de Aceite** | **Quantitativa:** 100% das telas aprovadas pelo stakeholder da BrPec (Marcos Ferreira, Gerente) em revisão formal de UI/UX ao final de cada sprint. Conformidade visual validada via design tokens definidos no guia de estilos do projeto. **Protocolo de aceite:** Apresentação de protótipos navegáveis (Figma ou similar) para validação prévia. Aprovação formal registrada em ata. Testes de compatibilidade cross-browser em Chrome 120+, Edge 120+, Safari 17+ e tablets Android (viewport 768px–1024px). |
...
| **RF/RN Associados** | Restrição organizacional (identidade visual definida pela equipe em alinhamento com o parceiro), Restrição técnica (plataforma web) |
| **Como será atendido** | Design system implementado com tokens CSS (CSS Custom Properties) para cores, tipografia e espaçamentos, definidos no guia de estilos elaborado pela equipe. Frontend em TypeScript. SPA responsiva com media queries para tablets (breakpoints 768px, 1024px). Validação contínua de UI com stakeholder em sprint reviews. |
...
<div align="center">
<p align="center">Figura 35 - Tela de seleção de perfil e login do supervisor</p>
<p align="center">
<img src="outros/assets/tela-login-supervisor.png" alt="Tela de seleção de perfil e login do supervisor" border="0">
</a>
</p>
<p align="center">Fonte: Próprios autores (2026).</p>
</div>
...
<div align="center">
<p align="center">Figura 36 - Tipografia</p>
<p align="center">
<img src="outros/assets/tela-home-e-delegar-tarefa-supervisor.png" alt="Tipografia" border="0">
</a>
</p>
<p align="center">Fonte: Próprios autores (2026).</p>
</div>
...
<div align="center">
<p align="center">Figura 37 - Tela tarefas e chamados de infraestrutura supervisor</p>
<p align="center">
<img src="outros/assets/tela-tarefas-e-chamados-de-infraestrutura-supervisor.png" alt="Tela tarefas e chamados supervisor" border="0">
</a>
</p>
<p align="center">Fonte: Próprios autores (2026).</p>
</div>

Link do Figma: https://www.figma.com/design/vrq7u1pMmPSpTdAUUYwxrV/Prot%C3%B3tipo-de-Alta-Fidelidade---BrPec?node-id=16-2&t=8SU4wKL9NuOS83IC-1
