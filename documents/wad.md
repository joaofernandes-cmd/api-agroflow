...
|---------|----|----|----------|------|-------|-----------|
| Capataz Daniel | RF001 | RN01 | `POST /movimentacoes`; `GET /movimentacoes/{id}`; `PATCH /movimentacoes/{id}`; `DELETE /movimentacoes/{id}` | Registro de movimentação | CT-RF001 (`movimentacao.spec.ts`) | Testes automatizados executados: `POST /movimentacoes` retornando HTTP 201 e objeto da movimentação; `GET` e `PATCH /movimentacoes/1` retornando HTTP 200; `DELETE /movimentacoes/1` retornando HTTP 204 |
| Supervisor Luiz | RF002 | RN02 | `POST /tarefas`; `GET /tarefas`; `PATCH /tarefas/{id}`; `PATCH /tarefas/{id}/status`; `DELETE /tarefas/{id}` | Criar e acompanhar tarefas | CT-RF002 (`tarefa.spec.ts`) | Testes automatizados executados: `POST /tarefas` retornando HTTP 201 com a tarefa criada; `PATCH /tarefas/11/status` retornando HTTP 200; `DELETE /tarefas/11` retornando HTTP 204 |
| Capataz Daniel | RF003 | RN03 | `GET /sincronizacao/conexao`; `POST /sincronizacao`; `GET /sincronizacao/status`; `GET /sincronizacao/mensagem`; `POST /movimentacoes/sincronizar`; `PATCH /movimentacoes/{id}/sincronizar` | Sincronização offline/online | CT-RF003 (`sincronizacao.spec.ts`; `movimentacao.spec.ts`) | Testes automatizados executados: `GET /sincronizacao/conexao` retornando HTTP 200 e `temConexao: true`; `POST /sincronizacao` retornando HTTP 200, `sucesso: true`, 3 registros sincronizados e lista de erros vazia; `PATCH /movimentacoes/1/sincronizar` retornando registro com `sincronizado: true` |
| Capataz Daniel / Supervisor Luiz | RF004 | RN04 | `GET /evidencias`; `GET /evidencias/{id}`; `POST /evidencias/fotos`; `POST /evidencias/audios`; `POST /evidencias/mensagens` | Anexar evidência | CT-RF004 (`evidencia.spec.ts`) | Testes automatizados executados: `POST /evidencias/fotos` com latitude `-19.9` e longitude `-57.6`, `/audios` e `/mensagens` retornando HTTP 201; resposta da foto contendo `foto.jpg`; `GET /evidencias/31` retornando HTTP 200 |
| Supervisor Luiz / Gerente Marcos | RF005 | RN05 | `POST /usuarios/login` | Login | CT-RF005 (`usuario.spec.ts`; `usuario.service.spec.ts`) | Figura 50 – `POST /usuarios/login` com token JWT retornado; teste automatizado executado retornando HTTP 200 e `token: mock-jwt-token`; cenário de Capataz bloqueado retornando HTTP 403 |
| Supervisor Luiz | RF006 | RN06 | `POST /validacoes/permissao`; `PATCH /validacoes/movimentacoes/{id}/validar`; `PATCH /validacoes/tarefas/{id}/aprovar`; `PATCH /validacoes/tickets/{id}/aprovar` | Validações pendentes | CT-RF006 (`validacao.spec.ts`; `usuario.service.spec.ts`) | Testes automatizados executados: `POST /validacoes/permissao` retornando HTTP 200 e `podeValidar: true`; aprovações de movimentação, tarefa e ticket retornando HTTP 200 e mensagens explícitas de sucesso |
| Gerente Marcos / Supervisor Luiz | RF007 | RN07 | `GET /relatorios/movimentacoes/dados`; `GET /relatorios/tarefas/dados`; `GET /relatorios/movimentacoes`; `GET /relatorios/semanal`; `GET /relatorios/mensal`; `GET /sincronizacao/relatorios/movimentacoes`; `GET /sincronizacao/relatorios/tarefas` | Relatórios | CT-RF007 (`relatorio.spec.ts`; `sincronizacao.spec.ts`) | Testes automatizados executados: `GET /relatorios/semanal` e `/mensal` retornando HTTP 200 com dados de movimentações; consultas sincronizadas de movimentações e tarefas retornando HTTP 200 |
| Capataz Daniel / Supervisor Luiz | RF008 | RN08 | `POST /tickets`; `GET /tickets/pendentes`; `GET /tickets/{id}`; `PATCH /tickets/{id}/atribuicao`; `PATCH /validacoes/tickets/{id}/aprovar` | Tickets de infraestrutura | CT-RF008 (`ticket.spec.ts`; `validacao.spec.ts`) | Testes automatizados executados: `POST /tickets` com `temEvidenciaDescritiva: true` retornando HTTP 201; `GET /tickets/pendentes` e `PATCH /tickets/21/atribuicao` retornando HTTP 200; aprovação retornando a mensagem `Ticket aprovado com sucesso.` |
...
&nbsp;&nbsp;&nbsp;&nbsp;As atualizações previstas para a Matriz de Rastreabilidade na Sprint 4 já foram contempladas a partir das implementações realizadas na Sprint 3. Durante essa etapa, os principais fluxos funcionais do sistema foram implementados no backend por meio de rotas REST, controllers, services, repositories e testes automatizados, permitindo associar cada requisito funcional às suas respectivas regras de negócio, endpoints, telas previstas e evidências de teste. Dessa forma, a matriz passou a refletir não apenas o planejamento do sistema, mas também os elementos efetivamente implementados, garantindo maior consistência entre escopo, regra de negócio, API e validação técnica.

&nbsp;&nbsp;&nbsp;&nbsp;A RTM evidencia que os fluxos centrais do sistema possuem rastreabilidade entre requisitos, regras de negócio, endpoints reais do backend, middlewares e testes automatizados. Os registros de movimentação, tarefas, tickets, evidências, autenticação, sincronização, validação, relatórios e dashboard estão conectados aos arquivos de teste de integração correspondentes, permitindo verificar a cobertura funcional durante a evolução do projeto.
