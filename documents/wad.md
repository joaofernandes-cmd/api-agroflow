...
    Identificou-se a necessidade de resolver os relacionamentos N:N (muitos-para-muitos) entre a tabela evidencia e as tabelas movimentacao, tarefa e ticket. Para isso, foram criadas três tabelas intermediárias (evidencia_movimentacao, evidencia_tarefa e evidencia_ticket), cada uma contendo dois campos: a chave estrangeira da tabela evidencia e a chave estrangeira da entidade correspondente. Pelo mesmo princípio, o relacionamento N:N entre Supervisores e Retiros foi resolvido por meio da tabela intermediária supervisor_retiro, que vincula um supervisor a um ou mais retiros sob sua coordenação, refletindo a possibilidade de um mesmo supervisor atuar em múltiplos retiros.
...
    As restrições de integridade foram aplicadas conforme as regras de negócio levantadas junto ao parceiro. A tabela movimentacao armazena os dados comuns do registro, enquanto os dados específicos de compra, venda, transferência, nascimento e morte foram separados em tabelas especializadas (movimentacao_compra, movimentacao_venda, movimentacao_transferencia, movimentacao_nascimento e movimentacao_morte), cada uma identificada pelo próprio movimentacao_id, que atua simultaneamente como chave primária e estrangeira. Ao campo login da tabela usuario foi atribuída a restrição UNIQUE, impedindo cadastros duplicados. O campo sincronizado recebeu valor padrão false nas tabelas operacionais, garantindo que todo registro criado em modo offline seja iniciado como não sincronizado, em conformidade com a RN03. Os campos que representam categorias ou estados fixos como tipo, status e prioridade foram definidos como ENUM, restringindo os valores aceitos àqueles previstos nas regras de negócio e impedindo inserções inválidas diretamente no banco. Para os casos em que o usuário precisa registrar uma opção não prevista entre os valores fixos, foram acrescentados os campos complementares tipo_outro, na tabela movimentacao, e categoria_outro, na tabela ticket, ambos do tipo texto livre, utilizados apenas quando o valor selecionado no respectivo ENUM corresponde à opção "outro".
    O controle de autenticação dos Capatazes em campo foi modelado na tabela acesso_capataz, que mantém o token de acesso de cada usuário (token_hash), seu estado de validade (ativo), as datas de expiração e criação e a referência ao usuario_id correspondente. Esse mecanismo permite gerenciar o acesso dos operadores de campo de forma independente das credenciais de login convencionais, suportando o uso offline e a posterior sincronização. Na tabela usuario, além das credenciais e do vínculo com o retiro, foi incluído o campo identificador, que armazena um código de identificação do operador adotado pelo parceiro.
...
CREATE TABLE movimentacao_compra (
    movimentacao_id UUID PRIMARY KEY REFERENCES movimentacao(id),
...
| Gerente Marcos / Supervisor Luiz | RF007 | RN07 | `GET /relatorios/movimentacoes/dados`; `GET /relatorios/tarefas/dados`; `GET /relatorios/tickets/dados`; `GET /relatorios/movimentacoes`; `GET /relatorios/semanal`; `GET /relatorios/mensal`; `GET /relatorios/exportar`; `GET /sincronizacao/relatorios/movimentacoes`; `GET /sincronizacao/relatorios/tarefas` | Relatórios | CT07 (`relatorio.spec.ts`; `sincronizacao.spec.ts`) | Testes automatizados executados: `GET /relatorios/semanal` e `GET /relatorios/mensal` retornando HTTP 200 com dados de movimentações; `GET /sincronizacao/relatorios/movimentacoes` e `GET /sincronizacao/relatorios/tarefas` retornando HTTP 200 |
